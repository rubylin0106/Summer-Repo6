#### 0628  note
# r, h is input variable #french bress 
...
# Cylinder5  r = 20, h = 14    V = pi(20^2)(14) # h is wrong 
#            .
#            .
#            .
# Cylinder20 r = 12.25, h = 9  V = pi((12.25)^2)(9) # r is not sauared 
...
##print paste (comment)
## if we want to mutiple havr to use * we have to use 3*(4) instead of 3(4)
volume_cylinder <- function(r, h) {
  V = pi*r^2*h
  print(paste("The volume of a cylinder with a radius of 4 and 
              a height of 6 is", 
...
## pow (x,y) 8^2
# if we not declare it will can default 
pow <- function(x, y) {
  result <- x^y
...
# The If else statement ## if give condition baesd on some command  else  something is down 
...
## so if q is greater 20 you print line 229 or if is not <20 somethindg esle  231 
