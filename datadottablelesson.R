...
flights2%>%
  slice(3) # slice is how tiderverse to find the row we want 
...
# data.table way
flights1[order(origin, -dest)] # order on origin 


# equivalent tidyverse way    
flights2 %>%
  select(year,month,day,carrier,origin,dest)%>%
  arrange(origin, desc(dest)) # arrange is tidyverse code 
...
# Range Method: Use : to select variables within a range of # not recommand 
# columns.
...
flights1[, !c("year", "month")] # year and month will be remove 
...
flights2%>%
  select(-year, -month) # deselect use -
...
# data.table way
flights1[origin == "JFK" & dest == "LAX", .(year, month, day, hour)] # select the thing you like to see 

# or (and more descriptive)  preferred method

flights1[origin == "JFK" & dest == "LAX", .(year, month, day, hour,
                                            origin, dest)] # just add the  origin, dest
...
# equivalent tidyverse way    
flights2 %>%
  mutate(gain = dep_delay - arr_delay) -> # mutate give the column a name 
  flights3
flights3
# Let's see the new column
View(flights3) # see all row 
...
flights1[, c("gain") := .(NULL)]  # this is how we can remove the variable  := .(NULL)
flights1
...
flights3 %>%
  select(-gain) ->
  flights2
flights2 # just deselect the variable by using -
...
flights1[, c("gain", "dist_km") := .(NULL, NULL)] # just do null on them 
flights1
...
# data.table way
flights1[, .(meandd = mean(dep_delay))] # meandd is just a name 
...
## data.table way
flights1[, .(.N)] # .N to produce the number of obs
...
flights1[carrier == "AmerAir", carrier := "AA"] #the name you want to change old name  new name 
flights1
...
flights2 %>%
  ...
  group_by(origin, dest) %>%  # use origin, dest by order
...
flights1[carrier == "AA", .(meanad = mean(arr_delay)),
         by = .(origin, dest)][order(origin, -dest)] # we only need the mean(arr_delay) and by origin, dest
