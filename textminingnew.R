...
text_tibble <- tibble(line = 14:26, text = text) # line = 14:26 location code from the r code line 
...
text_tibble %>%
  unnest_tokens(word, text) -> tibble2 # the line location only saw the first ten 
tibble2

# print(tibble2, n = 33) # see more (tibble2, n = 33)



# Step 3  Find frequencies for each word # how many time does it appear in the text 
tibble2%>%
  count(word, sort =TRUE) %>% 
 filter(n >= 1)  # it appear once and more than one (at least once )
# it still without the filter function 
tibble2%>%
  count(word, sort =TRUE)

# Step 4  Create a Data visual (Bar Graph) showing and comparing word
# frequencies 
# show the words of graph 
...
text1 <-   c("We thus define the tidy text format as being a table",
...
text_tibble1 <- tibble(line = 74:84, text1 = text1)

text_tibble1



# Step 2 Find line locations for each word in the text #what is the purpose doing it ?

text_tibble1 %>%
  unnest_tokens(word, text1) -> tibble3
tibble3
...
tibble3%>%
  count(word, sort =TRUE) %>% 
  filter(n >= 4) # at least 4 it mean equal to 4 or greater than 4 
...
text <- readLines(file.choose("poemtextfile")) # trying to read in the file (and we don't have the file )
text

inspect(docs)

# Now organize words and frequencies in a table

dtm <- TermDocumentMatrix(docs) # when table appear means we almost there 
m <- as.matrix(dtm)
...
docs <- tm_map(docs, toSpace, "/") # we don' want /@ 
...
wordcloud(words = d$word, freq = d$freq, min.freq = 1,
          ...
          colors=brewer.pal(8, "YlOrBr")) # the most impact
...
# Now Let's connect words found in the Emma book to 
# that suggest a sentiment of Joy !
tidy_books%>%
  filter(book == "Emma") %>%
  inner_join(nrc_joy) %>% 
  count(word, sort = TRUE) 
